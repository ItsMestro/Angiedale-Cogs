...
class KickBanMixin(MixinMeta):
            ...
            for user_id in user_ids:
                if entry.user.id == user_id:
                    if user_id in tempbans:
                        ...
                        errors[user_id] = ("User with ID {user_id} is already banned.").format(
                            user_id=user_id
                        )
        ...
        for user_id, member in members.items():
            try:
                ...
                if success:
                    ...
                    errors[user_id] = ("Failed to ban user {user_id}: {reason}").format(
            ...
            async with self.config.guild(guild).current_tempbans() as tempbans:
                if user_id in tempbans:
                    ...
                    try:
                        ...
                        errors[user_id] = ("User with ID {user_id} not found").format(
                            user_id=user_id
                        )
                        continue
                    except discord.Forbidden:
    ...
    async def voicekick(
        ...
        try:
            ...
            await ctx.send(("I am unable to kick this member from the voice channel."))
            return
        except discord.HTTPException:
            await ctx.send(("Something went wrong while attempting to kick that member."))
            return
        else:
            await modlog.create_case(
                self.bot,
            ...
            await ctx.send(("User has been kicked from the voice channel."))
    ...
    async def voiceunban(
        ...
        if needs_unmute and needs_undeafen:
            ...
            await ctx.send(("That user isn't muted or deafened by the server."))
            return
        ...
        await ctx.send(("User is now allowed to speak and listen in voice channels."))
    ...
    async def voiceban(self, ctx: commands.Context, member: discord.Member, *, reason: str = None):
        ...
        await ctx.send(("User has been banned from speaking or listening in voice channels."))
