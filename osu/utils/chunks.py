...
def parse_map_to_dict(bmap):
    ...
    bm_info["Cached"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S%z")
    ...
    bm_info["Title"] = findline("Title:")
    bm_info["Artist"] = findline("Artist:")
    bm_info["Mapper"] = findline("Creator:")
    bm_info["Diff name"] = findline("Version:")
    bm_info["HP"] = findline("HPDrainRate:")
    bm_info["CS"] = findline("CircleSize:")
    bm_info["OD"] = findline("OverallDifficulty:")
    bm_info["AR"] = findline("ApproachRate:")
    bm_info["SV"] = findline("SliderMultiplier:")
    bm_info["TR"] = findline("SliderTickRate:")
    ...
    for x in hitcircles:
        ...
        data = x.split(",", 4)
        objects.append(
            {"x": data[0], "y": data[1], "time": data[2], "type": data[3], "hitSound": data[4]}
        )

    bm_info["Hitobjects"] = objects
...
def oppai(bmap, window_length=30000, step_size=5000):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        while hitcircles:
            ...
            try:
                window = [x for x in hitcircles if int(x.split(",")[2]) < seek + window_length]
            except IndexError:
                raise ParseError("Unexpected line in [HitObjects] section")
            if len(window) == 0:
                results.append((seek, 0.0, 0.0, 0.0))
                seek = seek + 5000
                hitcircles = [x for x in hitcircles if int(x.split(",")[2]) > seek]
                continue
            out = "".join(window)
            ...
            oppai_out = subprocess.check_output([OPPAI_PATH, tmpdir + "/tmp.osu"])
