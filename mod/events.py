...
log = logging.getLogger("red.angiedale.mod.events")
    ...
    async def check_mention_spam(self, message):
        ...
        if mention_spam["kick"]:
            if mentions >= mention_spam["kick"]:
                try:
                    await guild.kick(author, reason=("Mention Spam (Autokick)"))
                except discord.HTTPException:
                    log.warning(
        ...
        if mention_spam["warn"]:
            if mentions >= mention_spam["warn"]:
                try:
                    await author.send(("Please do not mass mention people!"))
                except (discord.HTTPException, discord.Forbidden):
                    try:
                        await message.channel.send(
                            ("{member}, Please do not mass mention people!").format(
