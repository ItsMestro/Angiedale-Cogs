...
class Core:
    ...
    async def play_coin(self, ctx, bet, choice):
        message = await ctx.send(("The coin flips into the air..."))
        await asyncio.sleep(2)
        outcome = random.choice((("heads"), ("tails")))
        msg = ("The coin landed on {}!").format(outcome)
        return choice.lower() in outcome, bet, msg, message

    @game_engine("Cups", ("1", "2", "3"))
    async def play_cups(self, ctx, bet, choice):
        message = await ctx.send(("The cups start shuffling along the table..."))
        await asyncio.sleep(3)
        outcome = random.randint(1, 3)
        msg = ("The coin was under cup {}!").format(outcome)
    ...
    async def play_dice(self, ctx, bet):
        ...
        msg = ("The dice landed on {} and {} ({}).").format(die_one, die_two, outcome)
        return outcome in (2, 7, 11, 12), bet, msg, message

    @game_engine("Hilo", (("low"), ("lo"), ("high"), ("hi"), ("seven"), ("7")))
...
class Blackjack:
    ...
    async def blackjack_game(self, ctx, amount):
        ...
        condition2 = MessagePredicate.lower_contained_in((("hit"), ("stay")), ctx=ctx)
        ...
        if choice.content.lower() == ("double"):
    ...
    async def double_down(self, ctx, ph, dh, amount, condition2, message):
        try:
            ...
            if choice2.content.lower() == ("stay"):
                dh = self.dealer(dh)
                return ph, dh, amount, message
            elif choice2.content.lower() == ("hit"):
    ...
    def bj_embed(ctx, ph, dh, count1, initial=False, outcome=None):
        ...
        embed.add_field(
            name=("{}'s Hand").format(ctx.author.name),
            value=hand.format(", ".join(deck.fmt_hand(ph)), count1),
        )
        embed.add_field(
            name=("{}'s Hand").format(ctx.bot.user.name), value=hand.format(dealer_hand, count2)
        )
...
class War:
    ...
    async def war_game(self, ctx, bet):
        ...
        pred = MessagePredicate.lower_contained_in((("war"), ("surrender"), ("ffs")), ctx=ctx)
        try:
...
class Double:
    ...
    async def double_game(self, ctx, bet):
        ...
        while bet > 0:
            ...
            pred = MessagePredicate.lower_contained_in((("double"), ("cash out")), ctx=ctx)
            ...
            if resp.content.lower() == ("cash out"):
                break
            await asyncio.sleep(1)
    ...
    def double_embed(ctx, count, amount, outcome=None):
        ...
        embed = discord.Embed(colour=0xFF0000)
        embed.add_field(name=("{}'s Score").format(ctx.author.name), value=score)
        embed.add_field(name="\u200b", value=options, inline=False)
        if not outcome:
            embed.add_field(
