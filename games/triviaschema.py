...
TRIVIA_LIST_SCHEMA = Schema(
    {
        Optional("$schema"): And(str, error=("{key} key must be a text value.")),
        Optional("AUTHOR"): And(str, error=("{key} key must be a text value.")),
        Optional("CONFIG"): And(
            {
                Optional("max_score"): And(
                    int,
                    lambda n: n >= 1,
                    error=("{key} key in {parent_key} must be a positive integer."),
                ),
                Optional("timeout"): And(
                    Use(int_or_float),
                    lambda n: n > 0.0,
                    error=("{key} key in {parent_key} must be a positive number."),
                ),
                Optional("delay"): And(
                    Use(int_or_float),
                    lambda n: n >= 4.0,
                ...
                Optional("bot_plays"): Const(
                    bool, error=("{key} key in {parent_key} must be either true or false.")
                ),
                ...
                Optional("payout_multiplier"): And(
                    Use(int_or_float),
                    lambda n: n >= 0.0,
                    error=("{key} key in {parent_key} must be a non-negative number."),
                ),
                ...
                ALWAYS_MATCH: And(
                    lambda __: False,
                    error=("{key} is not a key that can be specified in {parent_key}."),
                ),
            },
            error=("{key} should be a 'key: value' mapping."),
        ),
        str: And(
            [str, int, bool, float],
            error=("Value of question {key} is not a list of text values (answers)."),
        ),
        # This matches any extra key and always fails validation
        # for the purpose of better error messages.
        MATCH_ALL_BUT_STR: And(
            lambda __: False,
            error=("A key of question {key} is not a text value."),
        ),
    },
    error=("A trivia list should be a 'key: value' mapping."),
)
