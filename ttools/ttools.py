...
class TTools(commands.Cog):
    ...
    async def pickside(self, ctx: commands.Context, side: str = None):
        """"""
        guild = self.bot.get_guild(809836974285783051)
        if not ctx.author in guild.members:
            return
        if not side:
            return await ctx.send(f"You need to tell me which side you want to be playing on with either `{ctx.clean_prefix}pickside A` or `{ctx.clean_prefix}pickside B`")
        if side.upper() != "A" and side.upper() != "B":
            return await ctx.send("Your choice of side has to be either `A` or `B`")
        
        serverkey = await self.config.guild(guild).sheet()
        if not serverkey:
            return
        
        sh = self.gs.open_by_key(serverkey)

        signup_discord_ids = sh.worksheet("RoundCounter").get("A1:H")

        i = 1
        for t in signup_discord_ids:
            if str(ctx.author.id) in t:
                sideA = "Side A"
                sideB = "Side B"
                text = None
                try:
                    if t[6] == side.upper() and t[3] == str(ctx.author.id):
                        return await ctx.send(f"You're already signed up to play side {side.upper()} this round")
                    elif t[7] == side.upper() and t[4] == str(ctx.author.id):
                        return await ctx.send(f"You're already signed up to play side {side.upper()} this round")
                    else:
                        text = (
                            f"Your team already have sides picked. This will invert them. Are you sure?\n\n"
                            f"__**Old > New**__\n"
                            f"{t[1]}: `{sideA if t[6].upper() == 'A' else sideB}` > `{sideB if t[6].upper() == 'A' else sideA}`\n"
                            f"{t[2]}: `{sideB if t[6].upper() == 'A' else sideA}` > `{sideA if t[6].upper() == 'A' else sideB}`\n"
                        )
                except IndexError:
                    pass
                ...
                if not text:
                    text = (
                        f"You'll be playing `{sideA if side.upper() == 'A' else sideB}` this round.\n"
                        f"Your teammate will play `{sideB if side.upper() == 'A' else sideA}`\n\n"
                        f"Is this correct?"
                    )

                msg: discord.Message = await ctx.channel.send(text)
                start_adding_reactions(msg, ReactionPredicate.YES_OR_NO_EMOJIS)
                pred = ReactionPredicate.yes_or_no(msg, ctx.author)
                try:
                ...
                if t[3] == str(ctx.author.id):
                    sh.worksheet("RoundCounter").update_cell(i, 7, side.upper())
                    sh.worksheet("RoundCounter").update_cell(i, 8, "A" if side.upper() == "B" else "B")
                else:
                    sh.worksheet("RoundCounter").update_cell(i, 8, side.upper())
                    sh.worksheet("RoundCounter").update_cell(i, 7, "A" if side.upper() == "B" else "B")

                return await ctx.send("Side submission sent!")
