...
class ModSettings():
    ...
    async def modset_showsettings(self, ctx: commands.Context):
        ...
        globaldata = await self.modconfig.all()
        track_all_names = globaldata["track_all_names"]
        msg = ""
        msg += ("Track name changes: {yes_or_no}\n").format(
            yes_or_no=("Yes") if track_all_names else ("No")
        )
        guild = ctx.guild
        if not guild:
        ...
        data = await self.modconfig.guild(guild).all()
        ...
        msg += ("Delete repeats: {num_repeats}\n").format(
            num_repeats=("after {num} repeats").format(num=delete_repeats)
            if delete_repeats != -1
            else ("No")
        )
        msg += ("Warn mention spam: {num_mentions}\n").format(
            num_mentions=("{num} mentions").format(num=warn_mention_spam)
            if warn_mention_spam
            else ("No")
        )
        msg += ("Kick mention spam: {num_mentions}\n").format(
            num_mentions=("{num} mentions").format(num=kick_mention_spam)
            if kick_mention_spam
            else ("No")
        )
        msg += ("Ban mention spam: {num_mentions}\n").format(
            num_mentions=("{num} mentions").format(num=ban_mention_spam)
            if ban_mention_spam
            else ("No")
        )
        msg += (
            ...
            else ("Mention Spam Strict: Only unique mentions will count\n")
        )
        msg += ("Respects hierarchy: {yes_or_no}\n").format(
            yes_or_no=("Yes") if respect_hierarchy else ("No")
        )
        msg += ("Delete delay: {num_seconds}\n").format(
            num_seconds=("{num} seconds").format(num=delete_delay)
            if delete_delay != -1
            else ("None")
        )
        msg += ("Reinvite on unban: {yes_or_no}\n").format(
            yes_or_no=("Yes") if reinvite_on_unban else ("No")
        )
        msg += ("Send message to users on kick/ban: {yes_or_no}\n").format(
            yes_or_no=("Yes") if dm_on_kickban else ("No")
        )
        if default_days:
            msg += ("Default message history delete on ban: Previous {num_days} days\n").format(
    ...
    async def mentionspam_warn(self, ctx: commands.Context, max_mentions: int):
        ...
        """
        mention_spam = await self.modconfig.guild(ctx.guild).mention_spam.all()
        if not max_mentions:
            if not mention_spam["warn"]:
                return await ctx.send(("Autowarn for mention spam is already disabled."))
            await self.modconfig.guild(ctx.guild).mention_spam.warn.set(False)
            return await ctx.send(("Autowarn for mention spam disabled."))

        if max_mentions < 1:
            return await ctx.send(("`<max_mentions>` must be 1 or higher to autowarn."))
    ...
    async def mentionspam_kick(self, ctx: commands.Context, max_mentions: int):
        ...
        if not max_mentions:
            if not mention_spam["kick"]:
                return await ctx.send(("Autokick for mention spam is already disabled."))
            await self.modconfig.guild(ctx.guild).mention_spam.kick.set(False)
            return await ctx.send(("Autokick for mention spam disabled."))

        if max_mentions < 1:
            return await ctx.send(("`<max_mentions>` must be 1 or higher to autokick."))
    ...
    async def mentionspam_ban(self, ctx: commands.Context, max_mentions: int):
        ...
        if not max_mentions:
            if not mention_spam["ban"]:
                return await ctx.send(("Autoban for mention spam is already disabled."))
            await self.modconfig.guild(ctx.guild).mention_spam.ban.set(False)
            return await ctx.send(("Autoban for mention spam disabled."))

        if max_mentions < 1:
            return await ctx.send(("`<max_mentions>` must be 1 or higher to autoban."))
    ...
    async def deleterepeats(self, ctx: commands.Context, repeats: int = None):
        ...
        if repeats is not None:
            if repeats == -1:
                await self.modconfig.guild(guild).delete_repeats.set(repeats)
                ...
                await ctx.send(
                    ("Messages repeated up to {num} times will be deleted.").format(num=repeats)
                )
            else:
                await ctx.send(
                    (
            ...
            repeats = await self.modconfig.guild(guild).delete_repeats()
            if repeats != -1:
                await ctx.send(
                    (
    ...
    async def reinvite(self, ctx: commands.Context):
        ...
        """
        guild = ctx.guild
        cur_setting = await self.modconfig.guild(guild).reinvite_on_unban()
        if not cur_setting:
    ...
    async def dm(self, ctx: commands.Context, enabled: bool = None):
        ...
        guild = ctx.guild
        if enabled is None:
            setting = await self.modconfig.guild(guild).dm_on_kickban()
            await ctx.send(
                ("DM when kicked/banned is currently set to: {setting}").format(setting=setting)
            )
            return
        await self.modconfig.guild(guild).dm_on_kickban.set(enabled)
        if enabled:
            await ctx.send(("Bot will now attempt to send a DM to user before kick and ban."))
        else:
            await ctx.send(
                ("Bot will no longer attempt to send a DM to user before kick and ban.")
            )
    ...
    async def defaultdays(self, ctx: commands.Context, days: int = 0):
        ...
        guild = ctx.guild
        if not (0 <= days <= 7):
            return await ctx.send(("Invalid number of days. Must be between 0 and 7."))
        await self.modconfig.guild(guild).default_days.set(days)
        await ctx.send(
            ("{days} days worth of messages will be deleted when a user is banned.").format(
                days=days
            )
        )
    ...
    async def defaultduration(
        ...
        *,
        ...
        """
        guild = ctx.guild
        await self.modconfig.guild(guild).default_tempban_duration.set(duration.total_seconds())
        await ctx.send(
            ("The default duration for tempbanning a user is now {duration}.").format(
